...
for GasCompent in [[0.34,0.23,0.31,0.105],[0.27,0.64,0.03,0.001]]:
    ...
    df=pd.DataFrame(data={'temperature':temperatureListX,'NO_Detail':NO_EndPoint_Detail_Temp_Np})          
    df.to_csv("DataAnalyse\\YangmeiGas\\simulationData.csv")
    C_NO_Detail=NO_EndPoint_Detail_Temp_Np.reshape(len(concentrationList),len(temperatureListX))
    plt.figure()
    for dataLine in C_NO_Detail:        
        plt.plot(temperatureListX,dataLine,'-')
    plt.show()